...
const REL_LABELS = {
  teacher:   { zh: '师徒',       en: 'Teacher → Student' },
  develop:   { zh: '继承与发展', en: 'Inherit & Develop' },
  critique:  { zh: '批判与反对', en: 'Critique & Opposition' },
  influence: { zh: '思想影响',   en: 'Influence' },
};

const YEAR_ANCHORS = [
  { y: -650, x: 0.00 },{ y: -500, x: 0.04 },{ y: -300, x: 0.13 },
  { y:    1, x: 0.19 },{ y:  500, x: 0.24 },{ y: 1000, x: 0.29 },
  { y: 1400, x: 0.35 },{ y: 1600, x: 0.43 },{ y: 1750, x: 0.52 },
  { y: 1850, x: 0.64 },{ y: 1900, x: 0.75 },{ y: 1950, x: 0.88 },
  { y: 1970, x: 1.00 },
];

function yearToRatio(year) {
  for (let i = 0; i < YEAR_ANCHORS.length - 1; i++) {
    const a = YEAR_ANCHORS[i], b = YEAR_ANCHORS[i + 1];
...
function yearToX(y) { return WORLD_PAD_L + yearToRatio(y) * (WORLD_W - WORLD_PAD_L - WORLD_PAD_R); }
...
const PERSONALITY_AXES = [
  { key: 'knowledge', left: 'R', right: 'E', leftLabel: '理性', rightLabel: '经验' },
  { key: 'order', left: 'O', right: 'F', leftLabel: '秩序', rightLabel: '自由' },
  { key: 'mode', left: 'C', right: 'A', leftLabel: '沉思', rightLabel: '行动' },
  { key: 'stance', left: 'S', right: 'B', leftLabel: '怀疑', rightLabel: '信念' },
];

const PERSONALITY_QUESTIONS = [
  { id: 'q1', axis: 'knowledge', leftValue: 'R', rightValue: 'E', question: '当你遇到一个复杂问题，你更相信哪种路径？', leftLabel: '先找清晰原则', rightLabel: '先看真实经验' },
  { id: 'q2', axis: 'knowledge', leftValue: 'R', rightValue: 'E', question: '你更容易被哪类解释打动？', leftLabel: '结构严密的论证', rightLabel: '贴近生活的观察' },
  { id: 'q3', axis: 'knowledge', leftValue: 'R', rightValue: 'E', question: '面对争论时，你会优先追问什么？', leftLabel: '概念是否一致', rightLabel: '事实是否成立' },
  { id: 'q4', axis: 'order', leftValue: 'O', rightValue: 'F', question: '你理想中的思想世界更像什么？', leftLabel: '一座有秩序的建筑', rightLabel: '一片可漫游的旷野' },
  { id: 'q5', axis: 'order', leftValue: 'O', rightValue: 'F', question: '当规则束缚了表达，你通常会？', leftLabel: '先理解规则的理由', rightLabel: '寻找突破规则的可能' },
  { id: 'q6', axis: 'order', leftValue: 'O', rightValue: 'F', question: '你更欣赏哪种哲学气质？', leftLabel: '建立体系', rightLabel: '打开可能性' },
  { id: 'q7', axis: 'mode', leftValue: 'C', rightValue: 'A', question: '思想对你最重要的作用是？', leftLabel: '让内心变得清明', rightLabel: '让现实发生改变' },
  { id: 'q8', axis: 'mode', leftValue: 'C', rightValue: 'A', question: '如果你发现一个真理，你更想？', leftLabel: '继续理解它', rightLabel: '把它付诸实践' },
  { id: 'q9', axis: 'mode', leftValue: 'C', rightValue: 'A', question: '你面对时代问题时，更像哪一种人？', leftLabel: '凝视深处的人', rightLabel: '走入现场的人' },
  { id: 'q10', axis: 'stance', leftValue: 'S', rightValue: 'B', question: '面对一个宏大的信念，你的第一反应是？', leftLabel: '先拆开检验', rightLabel: '先理解它为何动人' },
  { id: 'q11', axis: 'stance', leftValue: 'S', rightValue: 'B', question: '你更害怕哪一种思想状态？', leftLabel: '过早相信', rightLabel: '永远不敢相信' },
  { id: 'q12', axis: 'stance', leftValue: 'S', rightValue: 'B', question: '你更愿意把人生理解为？', leftLabel: '持续追问的过程', rightLabel: '逐渐承担的承诺' },
];
...
function Silhouette({ id, size }) {
  ...
  const r = (n) => { const x = Math.sin(seed * 1.37 + n * 2.9) * 10000; return x - Math.floor(x); };
  ...
  const tp = 16, fh = 22 + browBump, np = 30, mp = 36, cp = 42 + chin;
  ...
  return (
    <svg viewBox="0 0 64 64" width={size} height={size} style={{display:'block'}}>
      <defs><clipPath id={`c-${id}`}><circle cx="32" cy="32" r="31"/></clipPath></defs>
      <g clipPath={`url(#c-${id})`}>
        <rect width="64" height="64" fill={PAL.bgInk}/>
        <g transform={fwd === -1 ? 'translate(64 0) scale(-1 1)' : ''}>
          <path d={path} fill="rgba(242,236,220,0.7)"/>
        </g>
      </g>
      <circle cx="32" cy="32" r="30.5" fill="none" stroke={PAL.ring} strokeWidth="0.8"/>
    </svg>
  );
}
function Cameo({ id, en, size = 46 }) {
  ...
  if (!src || errored) return <Silhouette id={id} size={size}/>;
  return (
    <div style={{ width: size, height: size, position: 'relative' }}>
      ...
      <div style={{
        ...
        boxShadow: `inset 0 0 0 0.5px ${PAL.ring}`, pointerEvents: 'none',
      }}/>
    </div>
  );
}

function PhilosophyTimeline({ fixedHeight = null }) {
  const { ERAS, SCHOOLS, RELATIONS, PHILOSOPHERS } = window.PHI_DATA;
  ...
  const [savedPersonality, setSavedPersonality] = useState(() => {
    try { return JSON.parse(localStorage.getItem('phi-personality-result') || 'null'); }
    catch { return null; }
  });
  ...
  useEffect(() => { localStorage.setItem('phi-lang', lang); }, [lang]);
  ...
  const CORE_PHILOSOPHER_IDS = useMemo(() => new Set([
    'thales', 'pythagoras', 'heraclitus', 'parmenides', 'democritus', 'protagoras',
    'socrates', 'plato', 'aristotle', 'epicurus', 'zeno_citium', 'plotinus',
    'augustine', 'avicenna', 'averroes', 'aquinas', 'ockham',
    'machiavelli', 'montaigne', 'bacon',
    'hobbes', 'descartes', 'spinoza', 'locke', 'leibniz', 'berkeley', 'hume',
    'voltaire', 'rousseau', 'kant',
    'hegel', 'schopenhauer', 'mill', 'kierkegaard', 'marx', 'peirce', 'james',
    'nietzsche', 'frege', 'husserl', 'dewey', 'russell', 'wittgenstein', 'heidegger',
    'sartre', 'quine', 'adorno', 'rawls', 'foucault', 'habermas', 'derrida', 'kripke',
  ]), []);
  const visiblePhilosophers = useMemo(() => (
    density === 'curated'
      ? PHILOSOPHERS.filter(p => CORE_PHILOSOPHER_IDS.has(p.id))
      : PHILOSOPHERS
  ), [density, PHILOSOPHERS, CORE_PHILOSOPHER_IDS]);
  const schoolCounts = useMemo(() => Object.fromEntries(SCHOOLS.map(s => [
    s.id,
    visiblePhilosophers.filter(p => p.schools.includes(s.id)).length,
  ])), [SCHOOLS, visiblePhilosophers]);
  ...
  const nodes = useMemo(() => {
    ...
    const yMin = isMobile ? 42 : 60, yMax = h - (isMobile ? 120 : 70);
    const byYear = [...visiblePhilosophers].sort((a,b)=>a.year-b.year);
    const placed = [];
    const minDx = isMobile ? 116 : 130, minDy = isMobile ? 82 : 96;
    ...
    for (const p of byYear) {
      ...
      for (const dy of yTries) {
        ...
        if (!collides(x0, y)) { found = { x: x0, y }; break; }
      }
      ...
      if (!found) {
        for (let dx = 30; dx <= 900 && !found; dx += 30) {
          for (const sgn of [1, -1]) {
            ...
            for (const dy of yTries) {
              const y = Math.min(yMax, Math.max(yMin, baseY + dy));
              if (!collides(xx, y)) { found = { x: xx, y }; break; }
            }
            if (found) break;
          }
        }
      ...
      if (!found) found = { x: x0, y: baseY };
      placed.push({ id: p.id, x: found.x, y: found.y, data: p });
    }
  ...
  const focused = useMemo(() => {
    ...
    const nodeSet = new Set([focusId]); const edgeSet = new Set();
    edges.forEach((e, i) => {
      if (e.from === focusId || e.to === focusId) {
        edgeSet.add(i); nodeSet.add(e.from); nodeSet.add(e.to);
      }
    });
  ...
  const jumpToEra = (era) => {
    ...
    el.scrollTo({ left: Math.max(0, WORLD_PAD_L + r * (WORLD_W - WORLD_PAD_L - WORLD_PAD_R) - el.clientWidth/2), behavior: 'smooth' });
  };

  const jumpToSchool = (schoolId) => {
    const el = scrollerRef.current; if (!el || !schoolId) return;
    const first = Object.values(nodes)
      .filter(n => n.data.schools.includes(schoolId))
      .sort((a, b) => a.data.year - b.data.year)[0];
    if (first) {
      setSelectedId(null);
      el.scrollTo({ left: Math.max(0, first.x - el.clientWidth * 0.32), behavior: 'smooth' });
    }
  };
  ...
  const jumpToPhilosopher = (id) => {
    ...
    el.scrollTo({ left: Math.max(0, n.x - el.clientWidth * (isMobile ? 0.42 : 0.5)), behavior: 'smooth' });
  };
  ...
  useEffect(() => {
    ...
    const md = (e) => {
      ...
      down = true; startX = e.clientX; startLeft = el.scrollLeft; el.style.cursor = 'grabbing';
    };
    const mm = (e) => { if (!down) return; el.scrollLeft = startLeft - (e.clientX - startX); };
  ...
  const scrollByDir = (dx) => scrollerRef.current?.scrollBy({ left: dx, behavior: 'smooth' });
  const worldHeight = Math.max(420, viewportH);

  return (
    <div className="phi-root" style={{
      ...
      <EraBar eras={ERAS} onJump={jumpToEra} scrollX={scrollX} viewportW={viewportW} lang={lang} isMobile={isMobile}/>

      <div style={{ flex: 1, position: 'relative', minHeight: 0 }}>
        <div ref={scrollerRef} data-scroller="phi" style={{
          ...
          <div style={{ position: 'relative', width: WORLD_W, height: '100%', minHeight: worldHeight }}>
            <StarBackground h={worldHeight}/>
            <EraDividers eras={ERAS} h={worldHeight}/>

            <svg width={WORLD_W} height={worldHeight} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
              {visibleEdges.map((e, i) => {
                ...
                const baseOpacity = isMobile
                  ? (e.kind === 'influence' ? 0.035 : (canonLink ? 0.075 : 0.055))
                  : (e.kind === 'influence' ? 0.10 : (canonLink ? 0.20 : 0.14));
                const opacity = outFilter
                  ? (isMobile ? 0.012 : 0.025)
                  : (faded ? (isMobile ? 0.022 : 0.045) : (isFocused ? 0.9 : baseOpacity));
                return (
                  <path key={i} d={d} stroke={REL_COLORS[e.kind]}
                    strokeWidth={isFocused ? 1.55 : (isMobile ? (canonLink ? 0.55 : 0.42) : (canonLink ? 0.75 : 0.55))} fill="none"
                    strokeLinecap="round"
            ...
            {Object.values(nodes).map(n => {
              ...
              const portraitSize = isMobile
                ? (isFocused ? (isCanon ? 56 : 50) : (isCanon ? 44 : 38))
                : (isFocused ? (isCanon ? 62 : 54) : (isCanon ? 50 : 44));
              return (
                <div key={n.id} className="phi-node"
                  onMouseEnter={() => setHoveredId(n.id)} onMouseLeave={() => setHoveredId(null)}
                  onClick={() => setSelectedId(s => s === n.id ? null : n.id)}
                  style={{
                    position: 'absolute', left: n.x, top: n.y,
                    transform: 'translate(-50%, -50%)', cursor: 'pointer',
                    opacity: dim ? 0.16 : 1, transition: 'opacity .3s, transform .3s',
                    zIndex: isFocused ? 4 : (isCanon ? 3 : 2), width: isMobile ? 104 : 126,
                  }}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{
                      ...
                      boxShadow: isCanon
                        ? (isFocused
                          ? `0 0 0 1px ${PAL.gold}, 0 0 0 5px ${PAL.goldSoft}, 0 0 28px rgba(219,190,112,0.18), 0 12px 30px rgba(0,0,0,0.34)`
                          : `0 0 0 1px ${PAL.gold}, 0 0 0 4px ${PAL.goldFaint}, 0 8px 22px rgba(219,190,112,0.12)`)
                        : (isFocused ? `0 0 0 1.3px ${PAL.ink}, 0 6px 18px ${PAL.nodeHalo}` : `0 2px 6px ${PAL.nodeHalo}`),
                      transition: 'width .2s, height .2s, box-shadow .2s',
                      overflow: 'hidden', background: PAL.bgInk, position: 'relative',
                    }}>
                      <Cameo id={n.id} en={p.en} size={portraitSize}/>
                      {isCanon && (
                        <>
                          ...
                          <div aria-hidden style={{
                            ...
                            width: 1, height: 5, background: 'rgba(255,232,165,0.55)',
                    ...
                    {q && hoveredId === n.id && !selectedId && (
                      <div style={{
                        ...
                        background: PAL.bgInk, border: `1px solid ${PAL.inkLine}`,
                        fontSize: 11, color: PAL.ink, lineHeight: 1.35,
        ...
        <SchoolSelector schools={SCHOOLS} activeSchool={activeSchool}
          counts={schoolCounts}
          onSelect={(id) => { setActiveSchool(id); jumpToSchool(id); }}
          onClear={() => setActiveSchool(null)}
          lang={lang} isMobile={isMobile}/>

        {!isMobile && (
          <>
            <button onClick={() => scrollByDir(-600)} aria-label={lang === 'zh' ? '向左浏览' : 'Scroll left'} style={arrowStyle('left')}>
              <NavChevron dir="left"/>
            </button>
            <button onClick={() => scrollByDir(600)} aria-label={lang === 'zh' ? '向右浏览' : 'Scroll right'} style={arrowStyle('right')}>
        ...
        <div style={{
          ...
          {[
            ['curated', lang==='zh'?'精要':'Core', CORE_PHILOSOPHER_IDS.size],
            ['full', lang==='zh'?'全量':'Full', PHILOSOPHERS.length],
          ].map(([k,label,count]) => (
            <button key={k} onClick={() => setDensity(k)} style={{
              height: isMobile ? 34 : 'auto',
              minWidth: isMobile ? 58 : 64,
              padding: isMobile ? '0 8px' : '5px 10px 6px', cursor: 'pointer',
              background: density === k ? 'rgba(219,190,112,0.10)' : 'transparent',
              color: density === k ? PAL.ink : PAL.inkSoft,
              border: 'none', borderBottom: density === k ? `1px solid ${PAL.gold}` : '1px solid transparent',
              font: 'inherit', display: 'flex',
              flexDirection: isMobile ? 'row' : 'column',
              alignItems: 'center', justifyContent: 'center',
              gap: isMobile ? 4 : 1,
            }}>
              <span style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{label}</span>
              <span style={{ fontSize: 8.5, color: density === k ? PAL.gold : PAL.inkSoft, letterSpacing: '0.04em' }}>{count}</span>
            </button>
          ))}
        </div>
        ...
        {isMobile && (
          <MobileActionBar
            ...
            testOpen={testOpen}
            onToggleList={() => { setMobileListOpen(o => !o); setMobileLegendOpen(false); }}
            onToggleLegend={() => { setMobileLegendOpen(o => !o); setMobileListOpen(false); }}
            onOpenTest={openPersonalityTest}
          />
        )}

        <RelationLegend lang={lang} isMobile={isMobile} open={mobileLegendOpen}/>
        {!isMobile && (
          <MiniMap eras={ERAS} nodes={Object.values(nodes)} scrollX={scrollX}
            viewportW={viewportW} activeSchool={activeSchool} isInFilter={isInFilter}
            worldHeight={worldHeight}
            onSeek={(r) => { const el = scrollerRef.current; if (el) el.scrollTo({ left: r * WORLD_W - el.clientWidth/2, behavior:'smooth' }); }}
            lang={lang}/>
        )}

        {isMobile && mobileListOpen && (
          <MobilePhilosopherList
            eras={ERAS}
      ...
      {selected && (
        <DetailPanel philosopher={selected} lang={lang}
          onClose={() => setSelectedId(null)} schools={SCHOOLS}
      ...
      {testOpen && (
        <PersonalityTestPanel
          ...
          onJump={(id) => { setTestOpen(false); jumpToPhilosopher(id); }}
        />
      )}
    </div>
  );
...
function arrowStyle(dir) {
  return {
    ...
    [dir]: 16, width: 38, height: 38, borderRadius: '50%',
    background: 'rgba(10,15,30,0.38)',
    border: `1px solid rgba(242,236,220,0.16)`,
    color: 'rgba(242,236,220,0.76)', cursor: 'pointer',
    padding: 0,
    zIndex: 5, opacity: 0.86,
...
function NavChevron({ dir }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" aria-hidden="true"
      ...
      <path d="M7 4.5 L11.5 9 L7 13.5" fill="none" stroke="currentColor"
        strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}

function Header({ lang, setLang, isMobile = false, onOpenTest }) {
  return (
    <header style={{
      ...
      display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: isMobile ? 10 : 24,
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: isMobile ? 0 : 18, minWidth: 0 }}>
        <div style={{
          ...
          {lang === 'zh' ? '西方哲学史' : 'A History of Western Philosophy'}
        </div>
        {!isMobile && (
          <>
            <div aria-hidden style={{
              width: 1, height: 22, background: PAL.inkLine, alignSelf: 'center',
            }}/>
            <div style={{
        ...
        <div aria-label="Language" style={{
          ...
          {[['zh','中文'],['en','EN']].map(([k, l], i) => (
            <React.Fragment key={k}>
              {i > 0 && <span aria-hidden style={{ color: PAL.inkFaint }}>/</span>}
              <button onClick={() => setLang(k)} aria-pressed={lang === k} style={{
                padding: '3px 4px', cursor: 'pointer', border: 'none',
...
function SchoolSelector({ schools, activeSchool, counts, onSelect, onClear, lang, isMobile = false }) {
  ...
  useEffect(() => { setOpen(false); }, [activeSchool]);
  useEffect(() => {
    if (!open) return;
    ...
    <div ref={panelRef} className="phi-panel" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        <button onClick={() => setOpen(o => !o)} aria-expanded={open} style={{
          height: isMobile ? 34 : 30, minWidth: isMobile ? 82 : 76, maxWidth: isMobile ? 132 : 148, padding: '0 10px',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10,
          background: 'rgba(10,15,30,0.46)', color: active ? PAL.ink : PAL.inkSoft,
          ...
          backdropFilter: 'blur(10px)',
          cursor: 'pointer', fontSize: isMobile ? 13 : 12.5,
        }}>
      ...
      {open && (
        <div style={{
          ...
          maxHeight: isMobile ? '62vh' : 310, overflowY: 'auto',
          padding: isMobile ? '12px 0 18px' : '7px 0',
          background: 'rgba(8,12,24,0.88)', border: '1px solid rgba(242,236,220,0.18)',
          ...
          {schools.map(s => {
            ...
            return (
              <button key={s.id} disabled={disabled} onClick={() => {
                if (disabled) return;
                setOpen(false);
                onSelect(s.id);
              }} style={{
                width: '100%', padding: isMobile ? '10px 16px' : '7px 9px', border: 'none',
                ...
                fontFamily: 'inherit', fontSize: isMobile ? 14 : 12,
              }}>
                <span style={{
                  paddingLeft: 8, borderLeft: `2px solid ${on ? PAL.gold : 'transparent'}`,
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
...
function EraIcon({ id, size = 22 }) {
  ...
  switch (id) {
    case 'ancient': // Parthenon temple
      return (
        <svg {...common}>
          <path d="M3.2 9.2 L12 4.2 L20.8 9.2 Z"/>
          <path d="M4.5 10.6 H19.5"/>
          <path d="M5.4 12.2 H18.6"/>
          <path d="M6.2 12.2 V19"/>
          <path d="M9.1 12.2 V19"/>
          <path d="M12 12.2 V19"/>
          <path d="M14.9 12.2 V19"/>
          <path d="M17.8 12.2 V19"/>
          <path d="M4.1 19 H19.9"/>
          <path d="M3.2 21 H20.8"/>
        </svg>
      );
    case 'medieval': // Tall Gothic cathedral
      return (
        <svg {...common}>
          <path d="M12 2.8 L14.3 8.2 V21"/>
          <path d="M12 2.8 L9.7 8.2 V21"/>
          <path d="M6.2 21 V11.5 L8.5 8.4 L10.8 11.5"/>
          <path d="M17.8 21 V11.5 L15.5 8.4 L13.2 11.5"/>
          <path d="M10 21 V15 Q12 11.6 14 15 V21"/>
          <path d="M12 15.2 V21"/>
          <path d="M4.8 21 H19.2"/>
          <circle cx="12" cy="9.5" r="1.15"/>
          <path d="M12 6.4 V8"/>
          <path d="M11.2 7.2 H12.8"/>
        </svg>
      );
    case 'renaissance': // Dome, proportion, human measure
      return (
        <svg {...common}>
          <path d="M4 19 Q12 6 20 19"/>
          <path d="M4 19 H20"/>
          <path d="M12 6 V19"/>
          <circle cx="12" cy="12.5" r="4.1"/>
          <path d="M9.2 15.4 L12 8.2 L14.8 15.4"/>
          <path d="M7.8 12.3 H16.2"/>
          <path d="M10.4 4.8 H13.6"/>
        </svg>
      );
    case 'early_modern': // Telescope and orbit
      return (
        <svg {...common}>
          <path d="M4.5 14.5 L16.5 7.5"/>
          <path d="M15.5 6 L19.5 8.3 L17.2 11.9 L13.2 9.6 Z"/>
          <path d="M5.4 13.2 L8.3 17.8"/>
          <path d="M7 16.6 L4.7 20"/>
          <path d="M7 16.6 L11 19.4"/>
          <path d="M5.5 7.5 C7.7 4.5 12.8 3.8 16.3 6.3"/>
          <path d="M18.6 11.3 C18.8 15.8 14.9 19.7 10.3 19.6"/>
        </svg>
      );
    case 'modern': // Factory, machine, critique
      return (
        <svg {...common}>
          <path d="M4 21 V11.5 L8.6 14.2 V11.5 L13.2 14.2 V9.2 H20 V21"/>
          <path d="M15.2 9.2 V5.4 H18.8 V9.2"/>
          <path d="M7 17 H8.6"/>
          <path d="M11 17 H12.6"/>
          <path d="M15 17 H16.6"/>
          <circle cx="16.4" cy="5" r="1.5"/>
          <path d="M14.9 5 H13.4"/>
          <path d="M19.4 5 H20.7"/>
        </svg>
      );
    case 'contemporary': // Networked constellation
      return (
        <svg {...common}>
          <circle cx="5.5" cy="7" r="1.35"/>
          <circle cx="17.8" cy="5.8" r="1.35"/>
          <circle cx="12" cy="12.4" r="1.55"/>
          <circle cx="6.3" cy="18" r="1.35"/>
          <circle cx="18.6" cy="17.2" r="1.35"/>
          <circle cx="12.8" cy="20.2" r="0.9"/>
          <path d="M5.5 7 L12 12.4 L17.8 5.8"/>
          <path d="M6.3 18 L12 12.4 L18.6 17.2"/>
          <path d="M5.5 7 L6.3 18 L12.8 20.2 L18.6 17.2 L17.8 5.8"/>
          <path d="M12 12.4 L12.8 20.2"/>
        </svg>
      );
    default:
...
function EraBar({ eras, onJump, scrollX, viewportW, lang, isMobile = false }) {
  ...
  return (
    <div style={{
      ...
      {eras.map((e, i) => {
        ...
        return (
          <button key={e.id} onClick={() => onJump(e)} style={{
            flex: 1,
            minWidth: 0,
            ...
            <div style={{
              flexShrink: 0, color: isActive ? PAL.gold : PAL.inkSoft,
              opacity: isActive ? 1 : 0.55, transition: 'color .3s, opacity .3s',
              display: 'flex', alignItems: 'center',
            }}>
              <EraIcon id={e.id} size={isMobile ? 15 : 20}/>
            </div>
            <div style={{
              minWidth: 0,
              ...
              {!isMobile && (
                <span style={{ fontSize: 9, color: PAL.inkSoft, opacity: isActive ? 0.9 : 0.55,
                  whiteSpace: 'nowrap', flexShrink: 0,
                }}>
                  {e.start < 0 ? `${-e.start}B` : e.start}
                </span>
              )}
            </div>
            {isActive && (
...
function StarBackground({ h }) {
  const stars = useMemo(() => {
    const out = [];
    for (let i = 0; i < 320; i++) {
      const r = (n) => { const x = Math.sin(i * 9.17 + n) * 10000; return x - Math.floor(x); };
  ...
  return (
    <svg width={WORLD_W} height={h} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      <defs>
        <radialGradient id="phi-dust" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="rgba(219,190,112,0.10)"/>
          <stop offset="100%" stopColor="rgba(219,190,112,0)"/>
        </radialGradient>
      </defs>
      {YEAR_ANCHORS.filter((_, i) => i % 2 === 0).map((a, i) => (
        <ellipse key={`dust-${i}`} cx={yearToX(a.y)} cy={h * (0.26 + (i % 5) * 0.12)}
          rx={180 + (i % 3) * 80} ry={90 + (i % 4) * 24}
          fill="url(#phi-dust)" opacity="0.20"/>
      ))}
      {stars.map((s, i) => (
        <circle key={i} cx={s.x} cy={s.y} r={s.size}
          fill={s.warm ? '#dbc070' : '#f2ecdc'} opacity={s.op}/>
      ))}
    </svg>
  );
}

function EraDividers({ eras, h }) {
  return (
    <svg width={WORLD_W} height={h} style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {eras.map((e, i) => {
        if (i === 0) return null;
        ...
        return (
          <line key={e.id} x1={x} y1={0} x2={x} y2={h}
            stroke="rgba(242,236,220,0.12)" strokeWidth="1" strokeDasharray="1 8"/>
        );
      })}
    </svg>
  );
...
function MobileActionBar({ lang, listOpen, legendOpen, testOpen, onToggleList, onToggleLegend, onOpenTest }) {
  return (
    <div style={{
      ...
      {[
        [lang === 'zh' ? '列表' : 'List', listOpen, onToggleList],
        [lang === 'zh' ? '关系' : 'Relations', legendOpen, onToggleLegend],
        [lang === 'zh' ? '测试' : 'Test', testOpen, onOpenTest],
...
function RelationLegend({ lang, isMobile = false, open = true }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 9, letterSpacing: '0.26em', textTransform: 'uppercase',
        color: PAL.inkSoft, marginBottom: 8 }}>
        {lang === 'zh' ? '关系 Relations' : 'Relations'}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {Object.entries(REL_LABELS).map(([k, v]) => (
          <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: PAL.inkMid }}>
...
function MobilePhilosopherList({ eras, philosophers, lang, selectedId, onJump, onClose }) {
  ...
  return (
    <div className="phi-panel" style={{
      position: 'absolute', left: 0, right: 0, bottom: 0,
      maxHeight: '72vh', zIndex: 12,
      ...
      <div style={{
        ...
        <div style={{
          color: PAL.ink, fontSize: 15, letterSpacing: lang === 'zh' ? '0.06em' : 0,
        }}>{lang === 'zh' ? '哲学家列表' : 'Philosophers'}</div>
      ...
      {grouped.map(({ era, people }) => (
        <section key={era.id} style={{ paddingTop: 10 }}>
          <div style={{
            display: 'flex', alignItems: 'center', gap: 8,
            color: PAL.gold, marginBottom: 7,
          }}>
          ...
          <div style={{
            ...
            {people.map(p => {
              ...
              return (
                <button key={p.id} onClick={() => onJump(p.id)} style={{
                  minWidth: 0,
                  ...
                  <div style={{
                    ...
                    boxShadow: CANON_PHILOSOPHER_IDS.has(p.id)
                      ? `0 0 0 1px ${PAL.gold}, 0 0 0 3px ${PAL.goldFaint}`
                      : `0 0 0 1px ${PAL.ring}`,
                  }}>
                    <Cameo id={p.id} en={p.en} size={32}/>
                  </div>
...
function PersonalityTestPanel({ lang, isMobile, philosophers, savedResult, onClose, onComplete, onJump }) {
  ...
  return (
    <div style={{
      ...
      <div className="phi-panel" style={{
        ...
        <div style={{
          height: 2, background: 'rgba(242,236,220,0.10)', marginBottom: 18,
        }}>
          <div style={{
        ...
        {result ? (
          ...
          <div style={{
            ...
            gap: isMobile ? 14 : 18, alignItems: 'stretch',
          }}>
            <div style={{
              ...
              <div>
                ...
                <div style={{
                  marginTop: 18, fontSize: isMobile ? 21 : 25,
                  lineHeight: 1.45, color: PAL.ink,
                }}>{q.question}</div>
              </div>
              <div style={{ marginTop: 26 }}>
                <div style={{
                  ...
                  {[
                    { side: 'left', value: 1, label: q.leftLabel, mark: q.leftValue },
                    { side: 'right', value: 5, label: q.rightLabel, mark: q.rightValue },
                  ].map(option => (
                    <button key={option.side} onClick={() => choose(option.value)} style={{
                      minHeight: isMobile ? 88 : 112,
                      padding: isMobile ? '16px 15px' : '20px 18px',
                      border: '1px solid rgba(242,236,220,0.16)',
                      borderBottomColor: 'rgba(219,190,112,0.34)',
                      background: option.side === 'left'
                        ? 'linear-gradient(135deg, rgba(219,190,112,0.09), rgba(8,12,24,0.46))'
                        : 'linear-gradient(135deg, rgba(8,12,24,0.46), rgba(92,118,168,0.11))',
                      color: PAL.ink,
                      cursor: 'pointer',
                      textAlign: 'left',
            ...
            {step > 0 && (
              <button onClick={() => setStep(s => Math.max(0, s - 1))} style={{
                ...
                border: '1px solid rgba(242,236,220,0.16)',
                background: 'transparent', color: PAL.inkMid,
                cursor: 'pointer',
              }}>{lang === 'zh' ? '上一题' : 'Back'}</button>
            )}
          </div>
        )}
      </div>
...
function PersonalityResult({ result, philosophers, isMobile, lang, onRetake, onSave, saveStatus, onJump }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        <ShareCardPreview result={result} people={people} isMobile={isMobile}/>
        <div style={{
          display: 'grid',
          ...
          <button onClick={onSave} style={personalityButtonStyle(true)}>
            {lang === 'zh' ? '保存分享图' : 'Save Image'}
          </button>
          <button onClick={onRetake} style={personalityButtonStyle(false)}>
            {lang === 'zh' ? '重新测试' : 'Retake'}
          </button>
        </div>
        {saveStatus && (
      ...
      <div className="phi-panel" style={{
        ...
        <h2 style={{
          margin: '8px 0 6px', fontSize: isMobile ? 25 : 32,
          lineHeight: 1.15, fontWeight: 500, color: PAL.ink,
        }}>{type.name}</h2>
        <div style={{ color: PAL.inkMid, fontSize: 14, lineHeight: 1.7 }}>{type.subtitle}</div>
          ...
          <div style={{
            ...
            {people.map(p => (
              <button key={p.id} onClick={() => onJump(p.id)} style={{
                ...
                alignItems: 'center', gap: 8,
                minWidth: 0, maxWidth: isMobile ? '100%' : 188,
        ...
        <p style={{ color: PAL.inkMid, fontSize: 14, lineHeight: 1.85, margin: '18px 0 0' }}>{type.description}</p>
        <blockquote style={{
          margin: '16px 0 0', padding: '12px 14px',
          border: '1px solid rgba(242,236,220,0.10)',
          background: 'rgba(242,236,220,0.025)',
          color: PAL.ink, lineHeight: 1.75, fontSize: 14,
        }}>{type.message}</blockquote>
          ...
          {PERSONALITY_AXES.map(axis => {
            ...
            return (
              <div key={axis.key}>
                ...
                <div style={{
                  height: 3, marginTop: 5, position: 'relative',
                  background: 'rgba(242,236,220,0.12)',
                }}>
                  <div style={{
                    position: 'absolute', top: -3, left: `${pct}%`,
                    width: 9, height: 9, borderRadius: '50%',
                    background: PAL.gold, transform: 'translateX(-50%)',
...
function ShareCardPreview({ result, people, isMobile }) {
  const type = result.type;
  const titleSize = isMobile
    ? Math.max(18, 24 - Math.max(0, type.name.length - 7) * 1.4)
    : Math.max(20, 27 - Math.max(0, type.name.length - 7) * 1.6);
  ...
  return (
    <div style={{
      ...
      <div aria-hidden style={{
        position: 'absolute', left: '12%', right: '12%', top: '48%',
        height: 1,
      ...
      <div>
        ...
        <div style={{
          marginTop: isMobile ? 18 : 20, color: PAL.ink,
          fontSize: titleSize,
          lineHeight: 1.12, fontWeight: 500,
          whiteSpace: 'nowrap',
          overflow: 'hidden',
          letterSpacing: 0,
        }}>{type.name}</div>
        <div style={{
          marginTop: 8, color: PAL.inkMid,
          fontSize: isMobile ? 11.5 : 12,
          lineHeight: 1.48,
        }}>{type.subtitle}</div>
      </div>
      <div style={{ textAlign: 'center', padding: isMobile ? '10px 0' : '12px 0' }}>
        <div style={{
          margin: '0 auto', width: isMobile ? 86 : 94, height: isMobile ? 86 : 94, borderRadius: '50%',
          border: `1px solid ${PAL.gold}`, color: PAL.gold,
          overflow: 'hidden',
          boxShadow: `0 0 0 7px ${PAL.goldFaint}, 0 0 32px rgba(219,190,112,0.10)`,
          background: PAL.bgInk,
          position: 'relative',
        }}>
          {mainPerson && <Cameo id={mainPerson.id} en={mainPerson.en} size={isMobile ? 86 : 94}/>}
          <div aria-hidden style={{
        ...
        <div style={{ marginTop: 12, color: PAL.ink, fontSize: isMobile ? 13.5 : 14.5 }}>
          {people.map(p => p.zh || p.en).join(' · ')}
        </div>
      </div>
      <div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {type.keywords.map(k => <span key={k} style={{
            border: '1px solid rgba(242,236,220,0.13)',
            color: PAL.inkMid, padding: '2px 6px', fontSize: isMobile ? 9.5 : 10,
          }}>{k}</span>)}
        </div>
        <div style={{
          ...
          <div aria-hidden style={{
            ...
            border: '1px solid rgba(242,236,220,0.16)',
            background:
              'linear-gradient(90deg, rgba(242,236,220,0.18) 1px, transparent 1px), linear-gradient(rgba(242,236,220,0.18) 1px, transparent 1px)',
...
async function renderPersonalityCardToPng(result, philosophers) {
  ...
  ctx.lineWidth = 3;
  ctx.beginPath(); ctx.arc(0, 0, 116, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.beginPath(); ctx.arc(0, 0, 146, 0, Math.PI * 2); ctx.stroke();
  if (portrait) {
    ctx.save();
    ctx.beginPath(); ctx.arc(0, 0, 108, 0, Math.PI * 2); ctx.clip();
...
function MiniMap({ eras, nodes, scrollX, viewportW, onSeek, isInFilter, activeSchool, worldHeight }) {
  const ref = useRef(null);
  const mmW = 520; const mmH = 28;
  ...
  return (
    <div style={{
      ...
      background: 'rgba(8,12,24,0.55)', border: '1px solid rgba(242,236,220,0.16)',
      padding: '7px 10px', display: 'flex', alignItems: 'center',
      zIndex: 5, boxShadow: '0 12px 28px rgba(0,0,0,0.16)',
      backdropFilter: 'blur(12px)',
    }}>
      <svg ref={ref} width={mmW} height={mmH} onClick={onClickMap}
        style={{ cursor: 'pointer', display: 'block' }}>
        {/* era boundaries */}
        {eras.map((e, i) => {
          if (i === 0) return null;
          const x = (yearToX(e.start) / WORLD_W) * mmW;
          return <line key={e.id} x1={x} y1={3} x2={x} y2={mmH - 3}
            stroke="rgba(242,236,220,0.14)" strokeWidth="0.7" strokeDasharray="1 3"/>;
        })}
        {/* node scatter */}
        {nodes.map(n => {
          const inFilter = !activeSchool || isInFilter(n.data);
          return (
            <circle key={n.id} cx={(n.x / WORLD_W) * mmW}
        ...
        {/* viewport window */}
        <rect x={ratio * mmW} y={2} width={windowW} height={mmH - 4}
          fill="rgba(219,190,112,0.05)"/>
        <line x1={ratio * mmW} y1={1} x2={ratio * mmW + windowW} y2={1}
          stroke={PAL.gold} strokeWidth="1"/>
        <rect x={ratio * mmW} y={2} width={windowW} height={mmH - 4}
          fill="none" stroke="rgba(242,236,220,0.52)" strokeWidth="0.8"/>
      </svg>
    </div>
  );
}
...
const WORK_OVERRIDES = {
  thales: ['无著作传世'], pythagoras: ['无著作传世'], heraclitus: ['《论自然》（残篇）'],
  parmenides: ['《论自然》（残篇）'], democritus: ['《小宇宙秩序》（佚）'], protagoras: ['《论真理》（佚）'],
  epicurus: ['《致美诺寇的信》', '《主要教义》'], zeno_citium: ['《论自然》（佚）'],
  plotinus: ['《九章集》'], anselm: ['《宣讲》', '《为什么上帝成为人》'],
  avicenna: ['《治疗论》', '《救治论》', '《医学典范》'], averroes: ['《决定性论述》', '《不一致的不一致》'],
  ockham: ['《逻辑大全》', '《箴言书注》'], bacon: ['《新工具》', '《学术的进展》'],
  hobbes: ['《利维坦》', '《论公民》'], spinoza: ['《伦理学》', '《神学政治论》'],
  leibniz: ['《单子论》', '《神义论》', '《人类理智新论》'], berkeley: ['《人类知识原理》', '《海拉斯与斐洛诺斯三篇对话》'],
  rousseau: ['《社会契约论》', '《爱弥儿》', '《论人类不平等的起源》'],
  mill: ['《论自由》', '《功利主义》', '《逻辑体系》'], kierkegaard: ['《非此即彼》', '《恐惧与战栗》', '《致死的疾病》'],
  schopenhauer: ['《作为意志和表象的世界》', '《附录与补遗》'],
  frege: ['《概念文字》', '《算术基础》'], husserl: ['《逻辑研究》', '《观念》', '《欧洲科学的危机》'],
  russell: ['《数学原理》', '《哲学问题》', '《西方哲学史》'], dewey: ['《民主与教育》', '《经验与自然》'],
  sartre: ['《存在与虚无》', '《存在主义是一种人道主义》'], arendt: ['《人的境况》', '《极权主义的起源》', '《艾希曼在耶路撒冷》'],
  rawls: ['《正义论》', '《政治自由主义》'], foucault: ['《规训与惩罚》', '《词与物》', '《性史》'],
  habermas: ['《交往行为理论》', '《公共领域的结构转型》'], derrida: ['《论文字学》', '《声音与现象》', '《书写与差异》'],
  quine: ['《从逻辑的观点看》', '《词与物》'], kripke: ['《命名与必然性》', '《维特根斯坦论规则与私人语言》'],
  popper: ['《科学发现的逻辑》', '《开放社会及其敌人》'], kuhn: ['《科学革命的结构》'],
  gadamer: ['《真理与方法》'], levinas: ['《总体与无限》', '《异于存在》'],
...
const WORK_OVERRIDES_EN = {
  socrates: PROFILE_OVERRIDES.socrates.works_en, plato: PROFILE_OVERRIDES.plato.works_en,
  aristotle: PROFILE_OVERRIDES.aristotle.works_en, augustine: PROFILE_OVERRIDES.augustine.works_en,
  aquinas: PROFILE_OVERRIDES.aquinas.works_en, machiavelli: PROFILE_OVERRIDES.machiavelli.works_en,
  descartes: PROFILE_OVERRIDES.descartes.works_en, locke: PROFILE_OVERRIDES.locke.works_en,
  hume: PROFILE_OVERRIDES.hume.works_en, kant: PROFILE_OVERRIDES.kant.works_en,
  hegel: PROFILE_OVERRIDES.hegel.works_en, marx: PROFILE_OVERRIDES.marx.works_en,
  nietzsche: PROFILE_OVERRIDES.nietzsche.works_en, wittgenstein: PROFILE_OVERRIDES.wittgenstein.works_en,
...
function DetailPanel({ philosopher, lang, onClose, schools, isMobile = false }) {
  const p = philosopher;
  ...
  return (
    <div className="phi-panel" style={{
      ...
      padding: isMobile ? '20px 18px 22px' : '26px 24px 20px', boxSizing: 'border-box',
      backdropFilter: 'blur(16px)',
    }}>
      <div aria-hidden style={{
      ...
      <div style={{
        ...
        alignItems: 'center', columnGap: isMobile ? 12 : 15,
      }}>
        <div style={{
          width: isMobile ? 58 : 68, height: isMobile ? 58 : 68, flexShrink: 0, borderRadius: '50%',
          overflow: 'hidden',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: isCanon
            ? `0 0 0 1px ${PAL.gold}, 0 0 0 4px ${PAL.goldFaint}, 0 12px 28px rgba(0,0,0,0.28)`
            : `0 0 0 1px ${PAL.ring}, 0 10px 24px rgba(0,0,0,0.20)`,
        }}>
          <Cameo id={p.id} en={p.en} size={isMobile ? 54 : 64}/>
        </div>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{
            ...
            <span style={{
              fontSize: 13, fontStyle: 'italic', color: PAL.inkSoft,
              whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
      ...
      <ProfileSection title={lang === 'zh' ? '生平' : 'Life'} lang={lang}>{lifeText}</ProfileSection>
      <ProfileSection title={lang === 'zh' ? '核心思想' : 'Core Ideas'} lang={lang}>{idea}</ProfileSection>
      {works.length > 0 && (
        <ProfileSection title={lang === 'zh' ? '代表作' : 'Works'} lang={lang}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {works.map((w, i) => (
              <div key={i} style={{
                display: 'flex', alignItems: 'baseline', gap: 8,
...
function ProfileSection({ title, children, lang }) {
  return (
    <section style={{
...
Object.assign(window, { PhilosophyTimeline, PAL, Cameo });
